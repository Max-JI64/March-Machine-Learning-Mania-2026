...
print(f"✅ Ensemble V5 (XGBoost + LightGBM Soft Blending) - 사용 스레드 수: {N_JOBS}")
...
for val_year in val_seasons:
    ...
    xgb_preds_dict[val_year] = model_xgb.predict(X_val)
    ...
    lgb_preds_dict[val_year] = model_lgb.predict(X_val)
    
    print(f"   [Season {val_year}] XGB Brier: {brier_score_loss(y_val, xgb_preds_dict[val_year]):.4f} | LGB Brier: {brier_score_loss(y_val, lgb_preds_dict[val_year]):.4f}")

print("\n🚀 2단계: 최적 블렌딩 가중치 탐색...")
alpha_candidates = np.round(np.arange(0.5, 1.01, 0.05), 2)
best_alpha = 1.0
best_avg_brier = 1.0
...
for alpha in alpha_candidates:
    curr_briers = []
    for val_year in val_seasons:
        blend_pred = alpha * xgb_preds_dict[val_year] + (1 - alpha) * lgb_preds_dict[val_year]
        curr_briers.append(brier_score_loss(y_val_dict[val_year], blend_pred))
    
    avg_brier = np.mean(curr_briers)
    print(f"   [XGB {alpha*100:g}% + LGB {(1-alpha)*100:g}%] Avg Brier: {avg_brier:.5f}")
    
    if avg_brier < best_avg_brier:
        best_avg_brier = avg_brier
        best_alpha = alpha

print(f"\n🎯 [최적 가중치] XGB {best_alpha*100:g}% + LGB {(1-best_alpha)*100:g}% -> Avg Brier: {best_avg_brier:.5f}")

print("\n🚀 3단계: 연도별 최종 앙상블 성적 요약")
final_briers = []
for val_year in val_seasons:
    blend_pred = best_alpha * xgb_preds_dict[val_year] + (1 - best_alpha) * lgb_preds_dict[val_year]
    brier = brier_score_loss(y_val_dict[val_year], blend_pred)
    final_briers.append(brier)
    print(f"| {val_year} | {brier:.4f} |")

print("-" * 60)
print(f"🎯 최종 평균 Brier Score: {np.mean(final_briers):.4f}")
