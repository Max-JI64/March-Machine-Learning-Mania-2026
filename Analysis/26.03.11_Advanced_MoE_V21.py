...
def add_moe_interaction_features(matchup_df):
    """
    FFM 전용 비선형 교호작용(Interactions) 지표와 업셋 가능성 스코어 생성.
    기존 V20에서 병합된 T1, T2 기초 스탯의 Diff를 바탕으로 교호작용 변수를 만듭니다.
    """
    df = matchup_df.copy()
    ...
    if 'PowerComposite_Diff' in df.columns:
        # V17에서 만든 종합 파워 지표 차이를 Elo/NetRtg/등의 대리 지표로 씁니다.
    ...
    # 1) FFM(Factorization Machine) 시너지 강제 추출
...
# ==============================================================================
# 2. 데이터 엔진: V21 (V20 + J파트 모멘텀 지표 병합)
# ==============================================================================
def load_v21_total_data(gender='M'):
    print(f"🚀 {gender} V21 피처 통합 (V20 + J 파트 Momentum Decay/Interactions)...")
    ...
    # 5. [NEW] J 파트 교호작용(Interaction) 변수 생성
...
def apply_full_augmentation(train_df, features, target='Label', noise_scale=0.0357):
    """
    1. 대칭 스왑 (Symmetric Swap) : 팀1과 팀2 정보(Diff 부호반전, T1<->T2 교환)를 뒤집어 데이터 2배
    2. 연속형 데이터 가우시안 노이즈 (Gaussian Noise) : 스탯 수치에 지터(Jitter)가해 과적합 억제
    """
    # 1. 대칭 스왑
    ...
    # T1_ / T2_ 개별 컬럼들도 완전히 치환 (비율 기반 또는 특정 단일 스탯들 겹칠 때 대비)
    ...
    # J 파트 교호작용 변수의 경우 스왑/노이즈 부여 뒤에 다시 정합성을 위해 덮어씌울 수도 있으나 
    # 증강 데이터의 무결성(과적합 회피)을 위해 그대로 두는 것을 권장합니다.
    
    return df_aug

def label_smoothing(y_target, smoothing_val=0.05):
    """
    4. Label Smoothing : 극단 확률로 인한 Brier Score 페널티 예방용 타겟 보정 (0 -> 0.025, 1 -> 0.975)
    """
    return y_target * (1 - smoothing_val) + 0.5 * smoothing_val

# ==============================================================================
# 4. 7-모델 앙상블 및 정밀 보정 루프 진입 (V21)
# ==============================================================================
print("\n🔥 V21: Advanced MoE (J Part) & Full Data Augmentation (I Part)", flush=True)

df_m = load_v21_total_data('M')
df_w = load_v21_total_data('W')
...
print(f"\n🚀 총 {len(features)}개 피처로 7개 모델 앙상블 학습 시작...", flush=True)
print("| Year | Cat | XGB | LGB | HGB | ET | LR | Blend | Calib |", flush=True)
...
for val_year in seasons:
    ...
    m_hgb = HistGradientBoostingRegressor(max_iter=800, learning_rate=0.01, max_depth=5, random_state=SEED).fit(X_tr, y_tr_smooth, sample_weight=weights)
    p_hgb = np.clip(m_hgb.predict(X_va), 0, 1)
    print(" [완료]", flush=True)
    
    # 5. Extra Trees (기존 y_tr 라벨 우선 사용 - 트리기반 회귀)
    ...
    print(f"| {val_year} | {s_cat:.4f} | {s_xgb:.4f} | {s_lgb:.4f} | {s_hgb:.4f} | {s_et:.4f} | {s_blend:.4f} | {s_calib:.4f} |", flush=True)

print("-" * 110, flush=True)
print(f"🎯 최종 V21 (MoE J파트 피처 + 완전 스왑 & 노이즈 & 스무딩) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
