...
print(f"✅ CatBoost V6 (Individual/Expanded) - 사용 스레드 수: {N_JOBS}")
...
def objective(trial):
    params = {
        ...
        'verbose': False,
        'thread_count': N_JOBS,
        'bootstrap_type': 'Bernoulli'  # CatBoost Subsample 사용시 필요
    }
    
    noise_scale = trial.suggest_float('noise_scale', 0.01, 0.05)
    cv_briers = []
    
    # 확장된 검증 기간 중 마지막 3년을 표본으로 최적화
    for val_year in val_seasons[-3:]:
        ...
        # 예측값 클리핑 (CatBoostRegressor는 확률 형태가 아닐 수 있으므로)
...
print("\n🚀 1단계: 하이퍼파라미터 최적화 (CatBoost V6, 30 trials)...")
...
# ==============================================================================
# 6. 최종 Time-based CV 평가 (2013-2025 확장)
# ==============================================================================
print("\n🚀 2단계: 최적 설정(CatBoost V6) 데이터 전체 시계열 학습 및 단일 모델 평가...")
final_briers = []
...
print("-" * 60)
print(f"🎯 전체 평균 Brier Score (2013-2025): {np.mean(final_briers):.4f}")
