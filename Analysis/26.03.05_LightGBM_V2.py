...
# 1. 환경 설정
# ==============================================================================
N_JOBS = min(os.cpu_count(), 32) # 256개 이상의 코어에서 수레드 충돌 가능성 방지를 위해 32로 제한
print(f"✅ LightGBM V2 (Classifier) - 사용 스레드 수: {N_JOBS} (전체 코어: {os.cpu_count()})")
...
# ==============================================================================
# 5. 하이퍼파라미터 최적화 (V2: LGBMClassifier + 고정 타겟 활용)
# ==============================================================================
def objective(trial):
    ...
    params = {
        ...
        'deterministic': True, 'force_row_wise': True # 재현성을 위한 강제 설정
    }
    ...
    for val_year in val_seasons[-3:]:
        ...
        train_aug = augment_data(train_temp, features, target) # Default noise 0.03
        ...
        # LGBMClassifier는 continuous y를 지원하지 않을 수 있으므로, 
        # binary objective + LGBMRegressor를 활용하여 확률을 직접 예측합니다.
        # (LGBMRegressor with objective='binary' is mathematically correct for probability estimation)
        y_tr_smooth = label_smoothing(y_tr_aug)

        model = lgb.LGBMRegressor(**params)
        model.fit(
            X_tr_aug, y_tr_smooth, sample_weight=weights_aug,
            eval_set=[(X_val, y_val)],
            callbacks=[lgb.early_stopping(stopping_rounds=30, verbose=False)]
        )
        cv_briers.append(brier_score_loss(y_val, model.predict(X_val)))
    
    print(f"   [Trial {trial.number}] Brier: {np.mean(cv_briers):.4f}")
    return np.mean(cv_briers)

print("\n🚀 1단계: 하이퍼파라미터 최적화 (LightGBM V2, 50 trials)...")
print("   - Optuna 로그가 표시되는지 확인하십시오...")
