...
# ==============================================================================
# 1. 데이터 베이스 및 피처 구성 (V37 베이스라인 롤백 유지)
# ==============================================================================
...
def augment_v39(df, features, target='Label', noise_scale=0.0545):
    df_swap = df.copy()
...
# ==============================================================================
# 2. 모델 학습 엔진 (V37 최적 파라미터 완전 롤백 + 라운드 분리)
# ==============================================================================
# V38의 실패를 거울삼아 V37에 쓰였던 넉넉한 파라미터로 완전히 되돌립니다.
cat_params = {
...
def train_round_model(df_train_sub, val_year, features, is_validation_only=False, X_va=None):
    """지정된 라운드 서브셋에 대해서만 모델을 훈련하고, 예측을 수행합니다."""
    train_mask = (df_train_sub['Season'] < val_year)
    df_tr_aug = augment_v39(df_train_sub[train_mask], features, 'Label', noise_scale=BEST_NOISE)
    ...
    if is_validation_only and X_va is not None:
        p_cat = np.clip(m_cat.predict(X_va), 0, 1)
        p_xgb = np.clip(m_xgb.predict(X_va), 0, 1)
        p_lgb = np.clip(m_lgb.predict(X_va), 0, 1)
        return p_cat, p_xgb, p_lgb

    return None, None, None # 학습만 하고 버림

def train_and_eval_season_with_rounds(df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip_limits):
    val_mask = (df_train['Season'] == val_year)
    df_val = df_train[val_mask].copy()
    
    # 데이터 분할기: DayNum 139 기준 (134-135: Play-in, 136-137: R64, 138-139: R32 ㅡ> Early)
    # (143-144: Sweet16, 145-146: Elite8, 152: Final4, 154: Final ㅡ> Late)
    early_mask_tr = (df_train['DayNum'] <= 139)
    late_mask_tr  = (df_train['DayNum'] > 139)

    early_mask_va = (df_val['DayNum'] <= 139)
    late_mask_va  = (df_val['DayNum'] > 139)
    
    df_tr_early = df_train[early_mask_tr]
    df_tr_late  = df_train[late_mask_tr]
    
    X_va_early = df_val.loc[early_mask_va, features]
    X_va_late  = df_val.loc[late_mask_va, features]
    
    p_blend = np.zeros(len(df_val))
    
    # 1. Early Model (초반 라운드 예측 특화) 훈련 및 예측
    if len(X_va_early) > 0:
        # Early Model은 과거의 Early Round 경기만 보고 훈련합니다.
        train_mask_e = (df_tr_early['Season'] < val_year)
        df_tr_aug_e = augment_v39(df_tr_early[train_mask_e], features, 'Label', noise_scale=BEST_NOISE)
        weights_e = BEST_DECAY ** (val_year - df_tr_aug_e['Season'])
        weights_e /= weights_e.mean()
        
        m_cat_e = CatBoostRegressor(**cat_p).fit(df_tr_aug_e[features], label_smoothing(df_tr_aug_e['Label']), sample_weight=weights_e)
        m_xgb_e = XGBRegressor(**xgb_p).fit(df_tr_aug_e[features], label_smoothing(df_tr_aug_e['Label']), sample_weight=weights_e)
        m_lgb_e = LGBMRegressor(**lgb_p).fit(df_tr_aug_e[features], label_smoothing(df_tr_aug_e['Label']), sample_weight=weights_e)
        
        p_cat_e = np.clip(m_cat_e.predict(X_va_early), 0, 1)
        p_xgb_e = np.clip(m_xgb_e.predict(X_va_early), 0, 1)
        p_lgb_e = np.clip(m_lgb_e.predict(X_va_early), 0, 1)
        p_elo_e = df_val.loc[early_mask_va, 'EloWinProb'].values
        
        # 하드코딩 비율 그대로 적용
        p_blend_e = np.clip((0.6 * p_cat_e + 0.2 * p_xgb_e + 0.1 * p_lgb_e + 0.1 * p_elo_e), 0, 1)
        p_blend[early_mask_va] = p_blend_e

    # 2. Late Model (후반 라운드 예측 특화) 훈련 및 예측
    if len(X_va_late) > 0:
        # Late Model은 과거의 Late Round 경기만 보고 훈련합니다.
        train_mask_l = (df_tr_late['Season'] < val_year)
        df_tr_aug_l = augment_v39(df_tr_late[train_mask_l], features, 'Label', noise_scale=BEST_NOISE)
        weights_l = BEST_DECAY ** (val_year - df_tr_aug_l['Season'])
        weights_l /= weights_l.mean()
        
        m_cat_l = CatBoostRegressor(**cat_p).fit(df_tr_aug_l[features], label_smoothing(df_tr_aug_l['Label']), sample_weight=weights_l)
        m_xgb_l = XGBRegressor(**xgb_p).fit(df_tr_aug_l[features], label_smoothing(df_tr_aug_l['Label']), sample_weight=weights_l)
        m_lgb_l = LGBMRegressor(**lgb_p).fit(df_tr_aug_l[features], label_smoothing(df_tr_aug_l['Label']), sample_weight=weights_l)
        
        p_cat_l = np.clip(m_cat_l.predict(X_va_late), 0, 1)
        p_xgb_l = np.clip(m_xgb_l.predict(X_va_late), 0, 1)
        p_lgb_l = np.clip(m_lgb_l.predict(X_va_late), 0, 1)
        p_elo_l = df_val.loc[late_mask_va, 'EloWinProb'].values
        
        p_blend_l = np.clip((0.6 * p_cat_l + 0.2 * p_xgb_l + 0.1 * p_lgb_l + 0.1 * p_elo_l), 0, 1)
        p_blend[late_mask_va] = p_blend_l
...
# 3. 메인 프로세스
# ==============================================================================
print("\n🔥 V39: Round-based Split Modeling (M_Early, M_Late, W_Early, W_Late)", flush=True)

df_m = load_v39_data('M')
df_w = load_v39_data('W')
...
print(f"\n🚀 총 4개 분할 (초/후반) 학습 진행 중... (연속 모드)", flush=True)
...
for val_year in seasons:
    p_m, y_m = train_and_eval_season_with_rounds(df_m, val_year, features_m, cat_params, xgb_params, lgb_params, M_CLIP)
    s_m = brier_score_loss(y_m, p_m)
    
    p_w, y_w = train_and_eval_season_with_rounds(df_w, val_year, features_w, cat_params, xgb_params, lgb_params, W_CLIP)
...
print(f"🎯 연속 성능 증진 V39 (Round Split Target 0.12) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
