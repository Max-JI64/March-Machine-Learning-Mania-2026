...
def load_v33_data(gender='M'):
    ...
    # ==============================================================================
    # 🌟 V33 KEY FEATURES: Momentum & Tournament DNA Index (2차 오답 노트 발굴)
    # ==============================================================================
    
    # 1. Momentum_Upset_Risk: (-시드 격차) * 최근 30일 승률 격차
    # 시드가 높은 팀(약팀)인데, 최근 30일 승률이 정배팀보다 높으면 위험도가 커짐
    if 'Last30_WinRate_Diff' in df.columns:
        df['Momentum_Upset_Risk'] = (-df['SeedNum_Diff']) * df['Last30_WinRate_Diff']
    else:
        df['Momentum_Upset_Risk'] = 0

    # 2. Tournament_DNA_Risk: Elo 격차 * 역대 1라운드 업셋 점수 격차
    # 절대 전력(Elo) 차이는 큰데, 정배팀이 과거 1라운드에서 자주 미끄러지던 새가슴일 경우 패널티
    if 'First_Round_Upset_Score_Diff' in df.columns:
        df['Tournament_DNA_Risk'] = df['Elo_Diff'] * df['First_Round_Upset_Score_Diff']
    else:
        df['Tournament_DNA_Risk'] = 0
                
    return df.fillna(0)

def augment_v33(df, features, target='Label', noise_scale=0.0545):
    df_swap = df.copy()
    ...
    # 스왑 시 V33 Upset Features도 부호 반전
...
# ==============================================================================
# 2. 모델 학습 엔진 (Gender Separation, V29 Parameter Base)
# ==============================================================================
def train_and_eval_season(df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip_limits):
    ...
    df_tr_aug = augment_v33(df_train[train_mask], features, 'Label', noise_scale=BEST_NOISE)
...
# 3. 메인 프로세스
# ==============================================================================
print("\n🔥 V33: Momentum & Tournament DNA Index Injection", flush=True)

df_m = load_v33_data('M')
df_w = load_v33_data('W')

with open('selected_features_v7.json', 'r') as f:
    v7_features = json.load(f)

# V33 핵심 피처 추가 (V30 피처들은 제외)
...
print(f"\n🚀 분리 학습 시작! 2차 오답 분석(Momentum/DNA) 기반 파생 피처 주입됨...", flush=True)
...
print(f"🎯 최종 V33 (Momentum & DNA) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
