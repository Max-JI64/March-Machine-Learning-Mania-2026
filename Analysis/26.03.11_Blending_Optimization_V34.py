...
def augment_v34(df, features, target='Label', noise_scale=0.0545):
    df_swap = df.copy()
...
# ==============================================================================
# 🌟 V34 OPTIMIZATION: Blending Weight Optimizer 🌟
# ==============================================================================
def optimize_blend_weights(p_cat, p_xgb, p_lgb, p_elo, y_true):
    def loss_func(weights):
        # weights: [w_cat, w_xgb, w_lgb, w_elo]
        # 제약: 모든 가중치는 양수이며, 합은 1
        w = np.exp(weights) / np.sum(np.exp(weights)) # Softmax to bound 0-1 and sum=1
        p_blend = w[0]*p_cat + w[1]*p_xgb + w[2]*p_lgb + w[3]*p_elo
        p_blend = np.clip(p_blend, 0, 1) # Prevent float precision issue (> 1.0000000000000002)
        return brier_score_loss(y_true, p_blend)
    
    # 초기 가중치 (로그 공간)
    init_weights = np.array([np.log(0.6), np.log(0.2), np.log(0.1), np.log(0.1)])
    res = minimize(loss_func, init_weights, method='BFGS')
...
def train_and_eval_season(df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip_limits):
    ...
    df_tr_aug = augment_v34(df_train[train_mask], features, 'Label', noise_scale=BEST_NOISE)
    ...
    # 🌟 V34 동적 블렌딩 가중치 계산 (OOF가 아닌 Val Set 기준 최적화하여 힌트 얻기. 
    # 실제 K-Fold가 아니라 미래의 년도는 모른다고 가정하면 완벽하진 않으나, 
    # 연도별로 모델 의존도가 어떻게 달라지는지 볼 수 있음)
    best_weights = optimize_blend_weights(p_cat, p_xgb, p_lgb, p_elo, y_va.values)
    
    p_blend = np.clip((best_weights[0] * p_cat + best_weights[1] * p_xgb + best_weights[2] * p_lgb + best_weights[3] * p_elo), 0, 1)
    ...
    return p_clipped, y_va, best_weights

# ==============================================================================
# 3. 메인 프로세스
# ==============================================================================
print("\n🔥 V34: Blending Optimization (Scipy Minimize Dynamic Weights)", flush=True)

df_m = load_v34_data('M')
df_w = load_v34_data('W')

with open('selected_features_v7.json', 'r') as f:
    v7_features = json.load(f)

req_features = list(set(v7_features + ['EloWinProb', 'SeedWinProb', 'Elo_x_SRS_Diff', 'WinRate_x_Pyth_Diff', 'MasMean_Diff', 'Momentum_Upset_Risk', 'Tournament_DNA_Risk']))
...
for val_year in seasons:
    p_m, y_m, w_m = train_and_eval_season(df_m, val_year, features_m, cat_params, xgb_params, lgb_params, M_CLIP)
    s_m = brier_score_loss(y_m, p_m)
    weights_m_list.append(w_m)
    
    p_w, y_w, w_w = train_and_eval_season(df_w, val_year, features_w, cat_params, xgb_params, lgb_params, W_CLIP)
    s_w = brier_score_loss(y_w, p_w)
    weights_w_list.append(w_w)
...
print(f"🎯 최종 V34 (Blending Opt base) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
print(f"⏱️ 총 소요 시간: {(time.time()-START_TIME)/60:.1f}분", flush=True)
print("-" * 100, flush=True)

mean_w_m = np.mean(weights_m_list, axis=0)
mean_w_w = np.mean(weights_w_list, axis=0)

print("\n💡 [Scipy Minimize] 산출된 평균 최적 블렌딩 가중치")
print(f"기존 하드코딩 비율: Cat(0.600) | XGB(0.200) | LGB(0.100) | Elo(0.100)")
print(f"**Men's 최적 백분율**: Cat({mean_w_m[0]:.3f}) | XGB({mean_w_m[1]:.3f}) | LGB({mean_w_m[2]:.3f}) | Elo({mean_w_m[3]:.3f})")
print(f"**Women's 최적 백분율**: Cat({mean_w_w[0]:.3f}) | XGB({mean_w_w[1]:.3f}) | LGB({mean_w_w[2]:.3f}) | Elo({mean_w_w[3]:.3f})")
