...
# ==============================================================================
# 2. 모델 학습 엔진 (V43 Base + 🌟 V45 Weight Re-Optimization 🌟)
# ==============================================================================
...
def train_and_eval_season_with_opt(df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip_limits):
    ...
    init_weights = [np.log(0.6), np.log(0.2), np.log(0.1), np.log(0.1)]
    res = minimize(loss_func, init_weights, args=(p_cat, p_xgb, p_lgb, p_elo, y_va.values), method='Nelder-Mead')
...
# 3. 메인 프로세스
# ==============================================================================
print("\n🔥 V45: Final Weight Re-Optimization for V43 Feature Baseline", flush=True)
...
print(f"\n🚀 V43 피처 환경에서 새로운 황금 가중치 탐색 중...", flush=True)
...
for val_year in seasons:
    ...
    s_m = brier_score_loss(y_m, p_m); weights_history['M'].append(w_m)
    try:
        if val_year in df_w['Season'].values:
            ...
            s_w = brier_score_loss(y_w, p_w); weights_history['W'].append(w_w)
...
print(f"🎯 연속 성능 증진 V45 (Final Weight Optimization) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)

avg_m_w = np.mean(weights_history['M'], axis=0)
avg_w_w = np.mean(weights_history['W'], axis=0)
print(f"💡 Men's 새로운 황금 블렌딩 비율   : CatBoost {avg_m_w[0]:.3f}, XGBoost {avg_m_w[1]:.3f}, LightGBM {avg_m_w[2]:.3f}, Elo {avg_m_w[3]:.3f}", flush=True)
print(f"💡 Women's 새로운 황금 블렌딩 비율 : CatBoost {avg_w_w[0]:.3f}, XGBoost {avg_w_w[1]:.3f}, LightGBM {avg_w_w[2]:.3f}, Elo {avg_w_w[3]:.3f}", flush=True)
