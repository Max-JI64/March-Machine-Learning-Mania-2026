...
# ==============================================================================
# 1. 데이터 베이스 및 피처 구성 (V43 성별 특화 베이스)
# ==============================================================================
...
# ==============================================================================
# 2. 모델 학습 엔진 (V43 Base + 🌟 V44 Gender-Specific Hyperparameters 🌟)
# ==============================================================================
...
# 🌟 V44: 성별 맞춤형 하이퍼파라미터 (Tuned for Data Volume & Meta)
M_CAT_PARAMS = {
    'iterations': 1200, 'learning_rate': 0.02, 'depth': 6, 'l2_leaf_reg': 12.0,
    'subsample': 0.85, 'random_strength': 0.8, 'bagging_temperature': 0.6,
    'border_count': 128, 'thread_count': -1, 'random_seed': SEED, 'verbose': False
}
W_CAT_PARAMS = {
    'iterations': 800, 'learning_rate': 0.035, 'depth': 5, 'l2_leaf_reg': 8.0,
    'subsample': 0.9, 'random_strength': 0.7, 'bagging_temperature': 0.5,
    'border_count': 128, 'thread_count': -1, 'random_seed': SEED, 'verbose': False
}

M_XGB_PARAMS = {'n_estimators': 1200, 'learning_rate': 0.01, 'max_depth': 6, 'subsample': 0.8, 'colsample_bytree': 0.8, 'random_state': SEED, 'verbosity': 0}
W_XGB_PARAMS = {'n_estimators': 800, 'learning_rate': 0.015, 'max_depth': 4, 'subsample': 0.85, 'colsample_bytree': 0.9, 'random_state': SEED, 'verbosity': 0}

M_LGB_PARAMS = {'n_estimators': 1200, 'learning_rate': 0.01, 'num_leaves': 40, 'subsample': 0.75, 'n_jobs': -1, 'random_state': SEED, 'verbose': -1}
W_LGB_PARAMS = {'n_estimators': 800, 'learning_rate': 0.012, 'num_leaves': 24, 'subsample': 0.85, 'n_jobs': -1, 'random_state': SEED, 'verbose': -1}

def train_and_eval_season(df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip_limits, weights):
    train_mask = (df_train['Season'] != val_year); val_mask = (df_train['Season'] == val_year)
    df_tr_aug = augment_data(df_train[train_mask], features, 'Label', noise_scale=BEST_NOISE)
    X_tr, y_tr = df_tr_aug[features], df_tr_aug['Label']; y_tr_smooth = label_smoothing(y_tr, smoothing_val=0.05)
    ...
    p_blend = np.clip((weights[0] * p_cat + weights[1] * p_xgb + weights[2] * p_lgb + weights[3] * p_elo), 0, 1)
    iso = IsotonicRegression(out_of_bounds='clip'); iso.fit(p_blend, y_va)
...
# 3. 메인 프로세스
# ==============================================================================
print("\n🔥 V44: Gender-Specific Hyperparameter Tuning (Final Sprint to 0.12)", flush=True)

df_m = load_data('M'); df_w = load_data('W')
...
# V43 Gender-Specific Features
features_m = [f for f in full_req_features if f in df_m.columns]
features_m = [f for f in features_m if f not in ['Is_Major_Conf_Diff', 'Elo_Diff', 'LateSeason_Altitude_Fatigue_Diff', '3PAr_Diff', 'OR%_Diff']]
features_w = [f for f in full_req_features if f in df_w.columns]
features_w = [f for f in features_w if f not in ['MasMean_Diff', 'WinRate_vs_Top50_Diff', 'Upset_Value_Diff', 'Coach_Tenure_Diff', 'Rank_Trend_Slope_Diff', 'Is_Major_Conf_Diff']]

print(f"✅ Men's 학습 피처: {len(features_m)}개 | Women's 학습 피처: {len(features_w)}개")

seasons = sorted([int(s) for s in df_m['Season'].unique() if s >= 2012 and s != 2020])
final_results = []

print(f"\n🚀 성별 맞춤형 하이퍼파라미터 가동 중 (2012-2025)...", flush=True)
...
for val_year in seasons:
    p_m, y_m = train_and_eval_season(df_m, val_year, features_m, M_CAT_PARAMS, M_XGB_PARAMS, M_LGB_PARAMS, M_CLIP, M_WEIGHTS)
    s_m = brier_score_loss(y_m, p_m)
    try:
        if val_year in df_w['Season'].values:
            p_w, y_w = train_and_eval_season(df_w, val_year, features_w, W_CAT_PARAMS, W_XGB_PARAMS, W_LGB_PARAMS, W_CLIP, W_WEIGHTS)
            s_w = brier_score_loss(y_w, p_w); y_total = np.concatenate([y_m.values, y_w.values]); p_total = np.concatenate([p_m, p_w])
        else:
            s_w = 0.0; y_total = y_m.values; p_total = p_m
    except:
        s_w = 0.0; y_total = y_m.values; p_total = p_m
    s_total = brier_score_loss(y_total, p_total); final_results.append(s_total)
    print(f"| {val_year} | M: {s_m:.4f} | W: {s_w:.4f} | Total: {s_total:.4f} |", flush=True)

print("-" * 100, flush=True)
print(f"🎯 연속 성능 증진 V44 (Gender-Specific Tuned) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
