...
def load_v29_data(gender='M'):
    ...
    # Error Analysis를 위해 팀의 원래 ID 보존
...
def augment_v29(df, features, target='Label', noise_scale=0.0545):
    df_swap = df.copy()
...
# ==============================================================================
# 2. 모델 학습 엔진 및 오답 추출 (Error Analyzer)
# ==============================================================================
def train_and_eval_season_with_error_analysis(df, df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip_limits):
    train_mask = (df_train['Season'] < val_year)
    val_mask = (df_train['Season'] == val_year)
    
    df_tr_aug = augment_v29(df_train[train_mask], features, 'Label', noise_scale=BEST_NOISE)
    ...
    # 성별별로 다른 안전마진 적용 (Women은 정배 위주 = 덜 자름 / Men은 역배 위주 = 강하게 자름)
    p_clipped = np.clip(p_calib, gender_clip_limits[0], gender_clip_limits[1])
    
    # --- ERROR ANALYSIS LOGIC START ---
    # 실제 DF로 복원하여 예측값과 정답을 결합함
...
# ==============================================================================
# 3. 메인 프로세스 (V29: Error Analysis Dashboard)
# ==============================================================================
print("\n🔥 V29: Error Analysis Pipeline (Finding the Next Breakthrough + V28 Baseline)", flush=True)

df_m = load_v29_data('M')
df_w = load_v29_data('W')

with open('selected_features_v7.json', 'r') as f:
    v7_features = json.load(f)
req_features = list(set(v7_features + ['EloWinProb', 'SeedWinProb', 'Elo_x_SRS_Diff', 'WinRate_x_Pyth_Diff', 'MasMean_Diff']))
...
print(f"\n🚀 분리 학습 및 오답 노트 수집 시작...", flush=True)
print("| Year | Men's Brier | Women's Brier | Total Weighted Brier |", flush=True)

# 극단적 안전마진 설정
# Men은 이변이 심하므로 더 깎아내고, Women은 확신하므로 살려둠 (V27 + V28 융합)
# (이 방법이 틀리더라도, Error Analysis가 우리를 올바른 방향으로 이끌 것)
M_CLIP = (0.05, 0.95)
W_CLIP = (0.01, 0.99)

for val_year in seasons:
    p_m, y_m, err_m = train_and_eval_season_with_error_analysis(df_m, df_m, val_year, features_m, cat_params, xgb_params, lgb_params, M_CLIP)
    s_m = brier_score_loss(y_m, p_m)
    
    p_w, y_w, err_w = train_and_eval_season_with_error_analysis(df_w, df_w, val_year, features_w, cat_params, xgb_params, lgb_params, W_CLIP)
...
print(f"🎯 최종 V29 (Baseline) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
print(f"⏱️ 총 소요 시간: {(time.time()-START_TIME)/60:.1f}분", flush=True)
print("-" * 100, flush=True)

# --- SAVE WORST PREDICTIONS FOR ANALYSIS ---
df_all_errors = pd.concat(all_errors, ignore_index=True)
# 가장 심하게 틀린 상위 100건을 저장 (가장 자신 있게 오답을 낸 경우들)
df_worst = df_all_errors.sort_values(by='Brier_Loss', ascending=False).head(200)

csv_path = 'worst_predictions_v32.csv'
df_worst.to_csv(csv_path, index=False)
print(f"🚨 2차 오답 노트 생성 완료: {csv_path} (상위 200건의 실패 사례 수집)")
