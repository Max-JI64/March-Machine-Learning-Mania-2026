...
# ==============================================================================
print("\n🔥 V22: Fast Model Diet (CatBoost, XGBoost, LightGBM) & Advanced MoE & Full Augmentation", flush=True)
...
print(f"\n🚀 총 {len(features)}개 피처로 3개 최고속/고성능 트리 모델(CatBoost, XGBoost, LightGBM) 학습 시작...", flush=True)
print("| Year | Cat | XGB | LGB | Blend | Calib |", flush=True)
...
for val_year in seasons:
    ...
    # 3. LightGBM (V3 / V4 설정 기반 - ExtraTrees 대체용 초고속 모델)
    ...
    # Weighted Blend (CatBoost 70%, XGB 20%, LGB 10% - V6/V7의 CatBoost 우대 철학 반영)
    p_blend = np.clip((0.70*p_cat + 0.20*p_xgb + 0.10*p_lgb), 0, 1)
...
print(f"🎯 최종 V22 (Fast Diet: Cat/XGB/LGB + MoE 피처 + 완전 증강) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
