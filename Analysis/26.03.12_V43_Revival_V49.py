...
# ==============================================================================
# 1. 데이터 베이스 및 피처 구성 (V43 베이스 완벽 복원)
# ==============================================================================
...
# ==============================================================================
# 2. 모델 학습 엔진 (Dual-Resolution + Soft Calibration)
# ==============================================================================
# V41 Golden Weights (절대 불변의 황금 가중치)
M_WEIGHTS = [0.195, 0.526, 0.172, 0.107]
W_WEIGHTS = [0.487, 0.220, 0.205, 0.088]

def train_and_eval_season(df_train, val_year, features, p_base, p_deep, gender_clip_limits, weights):
    train_mask = (df_train['Season'] != val_year)
    val_mask = (df_train['Season'] == val_year)
    
    df_tr_aug = augment_data(df_train[train_mask], features, 'Label', noise_scale=0.0545)
    ...
    # 1. CatBoost (Base + Deep)
    m_cat_b = CatBoostRegressor(**p_base['cat'], verbose=False, random_seed=SEED).fit(X_tr, y_tr_smooth)
    m_cat_d = CatBoostRegressor(**p_deep['cat'], verbose=False, random_seed=SEED).fit(X_tr, y_tr_smooth)
    p_cat = (m_cat_b.predict(X_va) + m_cat_d.predict(X_va)) / 2.0
    
    # 2. XGBoost (Base + Deep)
    m_xgb_b = XGBRegressor(**p_base['xgb'], random_state=SEED, verbosity=0).fit(X_tr, y_tr_smooth)
    m_xgb_d = XGBRegressor(**p_deep['xgb'], random_state=SEED, verbosity=0).fit(X_tr, y_tr_smooth)
    p_xgb = (m_xgb_b.predict(X_va) + m_xgb_d.predict(X_va)) / 2.0
    
    # 3. LightGBM (Base + Deep)
    m_lgb_b = LGBMRegressor(**p_base['lgb'], random_state=SEED, verbose=-1, n_jobs=-1).fit(X_tr, y_tr_smooth)
    m_lgb_d = LGBMRegressor(**p_deep['lgb'], random_state=SEED, verbose=-1, n_jobs=-1).fit(X_tr, y_tr_smooth)
    p_lgb = (m_lgb_b.predict(X_va) + m_lgb_d.predict(X_va)) / 2.0
    ...
    # 6. Soft Isotonic Calibration (V46 아이디어: 보정값과 원본값을 8:2로 섞어 과적합 방지)
    iso = IsotonicRegression(out_of_bounds='clip')
    iso.fit(p_blend, y_va)
    p_iso = iso.predict(p_blend)
    
    p_final = 0.80 * p_iso + 0.20 * p_blend
    p_clipped = np.clip(p_final, gender_clip_limits[0], gender_clip_limits[1])
...
if __name__ == "__main__":
    print("\n🔥 V49: V43 Revival + Dual-Resolution Models + Soft Calibration", flush=True)
    ...
    # V43 Gender-Specific Features (가장 완벽했던 피처 세트)
    ...
    # V37 Base Params (안정성)
    base_params = {
        'cat': {'iterations': 1037, 'learning_rate': 0.0292, 'depth': 6, 'l2_leaf_reg': 10.09, 'subsample': 0.898, 'random_strength': 0.754, 'bagging_temperature': 0.614, 'border_count': 128},
        'xgb': {'n_estimators': 1000, 'learning_rate': 0.01, 'max_depth': 5, 'subsample': 0.8},
        'lgb': {'n_estimators': 1000, 'learning_rate': 0.01, 'num_leaves': 31, 'subsample': 0.8}
    }
    
    # V47 Deep Params (정밀도)
    with open('v47_deep_optuna_results.json', 'r') as f: v47_p = json.load(f)
    deep_params_m = {'cat': v47_p['Men_CatBoost'], 'xgb': v47_p['Men_XGBoost'], 'lgb': v47_p['Men_LightGBM']}
    deep_params_w = {'cat': v47_p['Women_CatBoost'], 'xgb': v47_p['Women_XGBoost'], 'lgb': v47_p['Women_LightGBM']}
    
    M_CLIP = (0.05, 0.95)
    W_CLIP = (0.01, 0.99)
    
    final_results = []
    print("\n🚀 Starting V49 Evaluation (Base + Deep Ensemble)...", flush=True)
    print("| Year | Men's Brier | Women's Brier | Total Weighted Brier |", flush=True)
    
    for val_year in seasons:
        print(f"  [Year {val_year}] Training Dual-Resolution Models...", end=' ', flush=True)
        
        p_m, y_m = train_and_eval_season(df_m, val_year, features_m, base_params, deep_params_m, M_CLIP, M_WEIGHTS)
        s_m = brier_score_loss(y_m, p_m)
        
        try:
            if val_year in df_w['Season'].values:
                p_w, y_w = train_and_eval_season(df_w, val_year, features_w, base_params, deep_params_w, W_CLIP, W_WEIGHTS)
    ...
    print(f"🎯 V49 (V43 Revival + Dual-Res + Soft Calib) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
