...
LABEL_SMOOTH = 0.01
NOISE_SCALE = 0.02
SEEDS = [42, 123, 456, 789, 2024, 7, 31, 99, 555, 1234]
CALIB_NFOLDS = 3
...
def label_smoothing(y_target, smoothing_val=0.01):
...
def train_single_seed_blend(X_tr, y_tr, X_va, elo_va, features, cat_p, xgb_p, lgb_p, weights, seed):
    """Train 6 models with one seed, return blended prediction."""
    set_seed(seed)
    ...
    p_blend = (weights[0]*p_cat + weights[1]*p_xgb + weights[2]*p_lgb +
               weights[3]*p_l1 + weights[4]*p_en + weights[5]*elo_va)
    return np.clip(p_blend, 0, 1)

def get_oof_for_isotonic(df_tr, features, cat_p, xgb_p, lgb_p, weights):
    """K-Fold OOF within training set for legitimate Isotonic calibration."""
    kf = KFold(n_splits=CALIB_NFOLDS, shuffle=True, random_state=42)
    oof_preds = np.zeros(len(df_tr))
    y_raw = df_tr['Label'].values.astype(float)

    for fold_idx, (tr_idx, va_idx) in enumerate(kf.split(df_tr)):
        fold_tr = df_tr.iloc[tr_idx].copy()
        fold_va = df_tr.iloc[va_idx]

        fold_tr_aug = augment_data(fold_tr, features, 'Label', noise_scale=NOISE_SCALE)
        X_fold_tr = fold_tr_aug[features]
        y_fold_tr = fold_tr_aug['Label'].values.astype(float)

        X_fold_va = fold_va[features]
        elo_fold_va = fold_va['EloWinProb'].values

        p_blend = train_single_seed_blend(
            X_fold_tr, y_fold_tr, X_fold_va, elo_fold_va,
            features, cat_p, xgb_p, lgb_p, weights, seed=42
        )
        oof_preds[va_idx] = p_blend

    return oof_preds, y_raw
...
def train_and_eval_season(df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip, weights):
    ...
    df_tr = df_train[train_mask].copy()
    X_va = df_train.loc[val_mask, features]
    y_va = df_train.loc[val_mask, 'Label']
    elo_va = df_train.loc[val_mask, 'EloWinProb'].values

    oof_preds, y_tr_raw = get_oof_for_isotonic(df_tr, features, cat_p, xgb_p, lgb_p, weights)
    iso = IsotonicRegression(out_of_bounds='clip', y_min=0.01, y_max=0.99)
    iso.fit(oof_preds, y_tr_raw)

    df_tr_aug = augment_data(df_tr, features, 'Label', noise_scale=NOISE_SCALE)
    X_tr_full = df_tr_aug[features]
    y_tr_full = df_tr_aug['Label'].values.astype(float)

    all_preds = []
    for seed in SEEDS:
        p_blend = train_single_seed_blend(
            X_tr_full, y_tr_full, X_va, elo_va,
            features, cat_p, xgb_p, lgb_p, weights, seed
        )
        all_preds.append(p_blend)

    p_raw = np.mean(all_preds, axis=0)
    p_calibrated = iso.predict(p_raw)
    p_clipped = np.clip(p_calibrated, gender_clip[0], gender_clip[1])
    return p_clipped, y_va

if __name__ == "__main__":
    print(f"\n🔥 V57: V55 Base + Legitimate K-Fold Isotonic Calibration ({CALIB_NFOLDS}-Fold OOF)", flush=True)
    ...
    print(f"Seeds: {len(SEEDS)} | Calib Folds: {CALIB_NFOLDS} | Label Smooth: {LABEL_SMOOTH} | Noise: {NOISE_SCALE}", flush=True)

    print("\n🚀 Starting V57 Evaluation...", flush=True)
    print("| Year | Men's Brier | Women's Brier | Total Weighted Brier |", flush=True)

    for val_year in seasons:
        print(f"  [Year {val_year}] 6-Model x 10 Seeds + K-Fold Isotonic...", end=' ', flush=True)
    ...
    print(f"🎯 V57 (V55 + K-Fold Isotonic) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
