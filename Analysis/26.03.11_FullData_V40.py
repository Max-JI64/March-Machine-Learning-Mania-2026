...
def augment_v40(df, features, target='Label', noise_scale=0.0545):
    df_swap = df.copy()
...
# ==============================================================================
# 2. 모델 학습 엔진 (V37 최적 파라미터 복구 + 🌟 전체 데이터 학습 CV 로직 🌟)
# ==============================================================================
def train_and_eval_season(df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip_limits):
    # 🌟 V40 혁신 지점: val_year 이전 데이터만 쓰는게 아니라, val_year 자체를 제외한 "모든 연도(미래+과거)"를 훈련 데이터로 삼음.
    # 이렇게 하면 훈련 데이터가 항상 21년치(기존 11년치 대비 약 2배)로 극대화됨.
    train_mask = (df_train['Season'] != val_year)
    val_mask = (df_train['Season'] == val_year)
    
    df_tr_aug = augment_v40(df_train[train_mask], features, 'Label', noise_scale=BEST_NOISE)
    
    # 시간 가중치(Decay) 로직 제거: 이제 미래 데이터도 혼재되어 있으므로, 
    # 시간 순서에 따른 패널티를 없애고 모든 연도의 농구 데이터를 평등하게 통합 학습!
    # weights = BEST_DECAY ** abs(val_year - df_tr_aug['Season']) 도 해볼 수 있으나, 일단 순수 평등 학습으로 볼륨 극대화 측정
    ...
    m_cat = CatBoostRegressor(**cat_p).fit(X_tr, y_tr_smooth) # sample_weight 제거
...
# 3. 메인 프로세스
# ==============================================================================
print("\n🔥 V40: Data Expansion (Leave-One-Season-Out Integration)", flush=True)

df_m = load_v40_data('M')
df_w = load_v40_data('W')
...
print(f"\n🚀 전체 년도 대상 볼륨 학습 진행 중... (연속 모드)", flush=True)
...
print(f"🎯 연속 성능 증진 V40 (Data Expansion Target 0.12) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
