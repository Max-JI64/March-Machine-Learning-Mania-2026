...
def train_and_eval_season(df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip, weights):
    ...
    for seed in SEEDS:
        set_seed(seed)
        df_tr_aug = augment_data(df_train[train_mask], features, 'Label', noise_scale=NOISE_SCALE)
...
if __name__ == "__main__":
    print(f"\n🔥 V55: Sharpened Predictions (LabelSmooth={LABEL_SMOOTH}, Noise={NOISE_SCALE}) + 6-Model + 10 Seeds", flush=True)
    ...
    print(f"Seeds: {len(SEEDS)} | Label Smoothing: {LABEL_SMOOTH} | Noise: {NOISE_SCALE}", flush=True)
    print("\n🚀 Starting V55 Evaluation...", flush=True)
    print("| Year | Men's Brier | Women's Brier | Total Weighted Brier |", flush=True)

    for val_year in seasons:
        print(f"  [Year {val_year}] Sharpened 6-Model x 10 Seeds...", end=' ', flush=True)
    ...
    print(f"🎯 V55 (Sharpened + 6-Model + 10 Seeds) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
