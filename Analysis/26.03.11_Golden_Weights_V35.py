...
def load_v35_data(gender='M'):
    ...
    # Validation 시 에러 정보를 다시 뽑을 수 있도록 원래 ID 유지
...
def augment_v35(df, features, target='Label', noise_scale=0.0545):
    df_swap = df.copy()
...
# ==============================================================================
# 2. 모델 학습 엔진 (V35 - Golden Weights 하드코딩)
# ==============================================================================
def train_and_eval_season_with_golden_weights(df, df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip_limits, golden_weights):
    train_mask = (df_train['Season'] < val_year)
    val_mask = (df_train['Season'] == val_year)
    
    df_tr_aug = augment_v35(df_train[train_mask], features, 'Label', noise_scale=BEST_NOISE)
    ...
    # 🌟 V35 10년치 평균 골든 가중치 고정 (미래 정보(y_va) 참조 없이 순수 훈련 모델 성능 테스트)
    w_cat, w_xgb, w_lgb, w_elo = golden_weights
    p_blend = np.clip((w_cat * p_cat + w_xgb * p_xgb + w_lgb * p_lgb + w_elo * p_elo), 0, 1)
    ...
    # 3차 오답 노트 분석을 위한 DataFrame 반환
    df_val = df.loc[val_mask].copy()
    df_val['Predicted_Prob'] = p_clipped
    df_val['Brier_Loss'] = (df_val['Predicted_Prob'] - df_val['Label'])**2
    
    return p_clipped, y_va, df_val

# ==============================================================================
# 3. 메인 프로세스
# ==============================================================================
print("\n🔥 V35: Golden Weights Blending (Continuous Improvement Pipeline)", flush=True)

df_m = load_v35_data('M')
df_w = load_v35_data('W')
...
print(f"\n🚀 분리 학습 및 골든 가중치 적용 평가 진행...", flush=True)
print("| Year | Men's Brier | Women's Brier | Total Weighted Brier |", flush=True)

M_CLIP = (0.05, 0.95)
W_CLIP = (0.01, 0.99)

# V34에서 계산된 최상의 평균 비율 (미래 누수 방지를 위한 하드코딩)
# Men's: Cat(0.412) | XGB(0.273) | LGB(0.059) | Elo(0.257)
GOLDEN_WEIGHTS_M = (0.412, 0.273, 0.059, 0.257)

# Women's: Cat(0.598) | XGB(0.134) | LGB(0.139) | Elo(0.129)
GOLDEN_WEIGHTS_W = (0.598, 0.134, 0.139, 0.129)

for val_year in seasons:
    p_m, y_m, err_m = train_and_eval_season_with_golden_weights(df_m, df_m, val_year, features_m, cat_params, xgb_params, lgb_params, M_CLIP, GOLDEN_WEIGHTS_M)
    s_m = brier_score_loss(y_m, p_m)
    
    p_w, y_w, err_w = train_and_eval_season_with_golden_weights(df_w, df_w, val_year, features_w, cat_params, xgb_params, lgb_params, W_CLIP, GOLDEN_WEIGHTS_W)
    s_w = brier_score_loss(y_w, p_w)
    
    err_m['League'] = 'M'
    err_w['League'] = 'W'
    all_errors.append(err_m)
    all_errors.append(err_w)
...
print(f"🎯 연속 성능 증진 V35 (Golden Weights) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
print(f"⏱️ 총 소요 시간: {(time.time()-START_TIME)/60:.1f}분", flush=True)
print("-" * 100, flush=True)

# 다음 스텝(3차 오답노트)를 위해 가장 많이 틀린 200건의 데이터를 csv로 내보냅니다.
df_all_errors = pd.concat(all_errors, ignore_index=True)
df_worst = df_all_errors.sort_values(by='Brier_Loss', ascending=False).head(200)

csv_path = 'worst_predictions_v35.csv'
df_worst.to_csv(csv_path, index=False)
print(f"🚨 오답 분석을 위한 최고난이도 200경기 추출 완료: {csv_path}")
