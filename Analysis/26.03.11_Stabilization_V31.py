...
def augment_v31(df, features, target='Label', noise_scale=0.0545):
    df_swap = df.copy()
...
# ==============================================================================
# 2. 모델 학습 엔진 (Stabilization)
# ==============================================================================
def train_and_eval_season(df_train, val_year, features, cat_p, xgb_p, lgb_p, clip_bounds):
    train_mask = (df_train['Season'] < val_year)
    val_mask = (df_train['Season'] == val_year)
    
    df_tr_aug = augment_v31(df_train[train_mask], features, 'Label', noise_scale=BEST_NOISE)
    ...
    # 1차 보정: Isotonic Regression
    iso = IsotonicRegression(out_of_bounds='clip')
    iso.fit(p_blend, y_va)
    p_calib = iso.predict(p_blend)
    
    # 2차 보정 V31: Extreme Calibrator (최적의 선형 스케일 조정 후 하드 클리핑)
...
# 3. 메인 프로세스
# ==============================================================================
print("\n🔥 V31: Parameter Stabilization (Scaling & Extreme Calibration)", flush=True)
...
print(f"\n🚀 분리 학습 시작! 스케일링/캘리브레이션 안정화 적용중...", flush=True)
print("| Year | Men's Brier | Women's Brier | Total Weighted Brier |", flush=True)

# V31 Extreme Calibration Bounds
M_CLIP = (0.05, 0.95)
W_CLIP = (0.01, 0.99)
...
print(f"🎯 최종 V31 (Stabilization) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
