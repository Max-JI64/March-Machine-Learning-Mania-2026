...
def load_v31_data(gender='M'):
    ...
    # 🌟 V31 STABILIZATION: Upset Vulnerability 정규화 (스케일링 적용) 🌟
    if 'Margin_Std_Diff' in df.columns and 'TOV%_Diff' in df.columns:
        var_pen_raw = df['Margin_Std_Diff'] * df['TOV%_Diff']
        ups_idx_raw = (-df['SeedNum_Diff']) * df['Margin_Std_Diff']
        
        # 극단값을 방지하기 위해 Percentile Rank 변환 사용
        df['Variance_Penalty'] = var_pen_raw.rank(pct=True) * 2 - 1  # -1 ~ 1 Range
        df['Upset_Danger_Index'] = ups_idx_raw.rank(pct=True) * 2 - 1
    else:
        df['Variance_Penalty'] = 0
        df['Upset_Danger_Index'] = 0
                
    return df

def augment_v31b(df, features, target='Label', noise_scale=0.0545):
    df_swap = df.copy()
    ...
    for f in ['Variance_Penalty', 'Upset_Danger_Index']:
...
# ==============================================================================
# 🌟 V31 STABILIZATION: Scipy Extreme Calibration Optimizer 🌟
# ==============================================================================
class ExtremeCalibrator:
    def __init__(self, bounds=(0.01, 0.99)):
        self.bounds = bounds
        self.w = np.array([1.0, 0.0]) # [Scale, Offset]

    def _apply(self, probs, w):
        p_scaled = probs * w[0] + w[1]
        return np.clip(p_scaled, self.bounds[0], self.bounds[1])

    def fit(self, probs, y_true):
        def loss_fn(w):
            p_adj = self._apply(probs, w)
            return brier_score_loss(y_true, p_adj)
        res = minimize(loss_fn, self.w, method='Nelder-Mead')
        self.w = res.x
        return self

    def predict(self, probs):
        return self._apply(probs, self.w)

# ==============================================================================
# 2. 모델 학습 엔진 (Stabilization B: 2차 안정화)
# ==============================================================================
def train_and_eval_season_v31b(df_train, val_year, features, cat_p, xgb_p, lgb_p, clip_bounds):
    train_mask = (df_train['Season'] < val_year)
    val_mask = (df_train['Season'] == val_year)
    
    df_tr_aug = augment_v31b(df_train[train_mask], features, 'Label', noise_scale=BEST_NOISE)
    ...
    p_calib = iso.predict(p_blend)
    
    ext_cal = ExtremeCalibrator(bounds=clip_bounds).fit(p_calib, y_va)
    p_final = ext_cal.predict(p_calib)
...
# 3. 메인 프로세스
# ==============================================================================
print("\n🔥 V31-B: 2차 안정화 (Regulaziation) - Overfitting 방지 강력 규제", flush=True)

df_m = load_v31_data('M')
df_w = load_v31_data('W')

with open('selected_features_v7.json', 'r') as f:
    v7_features = json.load(f)

req_features = list(set(v7_features + ['EloWinProb', 'SeedWinProb', 'Elo_x_SRS_Diff', 'WinRate_x_Pyth_Diff', 'MasMean_Diff', 'Variance_Penalty', 'Upset_Danger_Index']))
...
# ==============================================================================
# 🤖 V31-B 하이퍼파라미터 변경점 (Regularization 강화)
# 나무 결절(Depth)을 낮추고, L2 패널티를 올려 '오버피팅' 차단
# ==============================================================================
cat_params = {
    'iterations': 1000, 
    'learning_rate': 0.02, 
    'depth': 5, # 기존 6에서 5로 축소 (너무 깊게 파고들지 못하게)
    'l2_leaf_reg': 30.0, # 기존 10.09에서 대폭 상승 (가중치 쏠림 강력 억제)
    'subsample': 0.85, 
    'random_strength': 0.8, 
...
xgb_params = {
    'n_estimators': 800, 
    'learning_rate': 0.015, 
    'max_depth': 4, # 기존 5에서 4로 축소
    'reg_lambda': 10.0, # L2 페널티 추가
    'reg_alpha': 2.0,   # L1 페널티 추가
    'subsample': 0.8, 
    'random_state': SEED, 
    'verbosity': 0
}
...
lgb_params = {
    'n_estimators': 800, 
    'learning_rate': 0.015, 
    'num_leaves': 15, # 깊이 통제 (기존 31에서 대폭 축소)
    'reg_lambda': 10.0, # L2 페널티 추가
    'reg_alpha': 2.0,
    'subsample': 0.8, 
    'n_jobs': -1, 
...
print(f"\n🚀 분리 학습 시작! Overfitting 방지용 강력한 L2/L1 페널티 활성화...", flush=True)
...
for val_year in seasons:
    p_m, y_m = train_and_eval_season_v31b(df_m, val_year, features_m, cat_params, xgb_params, lgb_params, M_CLIP)
    s_m = brier_score_loss(y_m, p_m)
    
    p_w, y_w = train_and_eval_season_v31b(df_w, val_year, features_w, cat_params, xgb_params, lgb_params, W_CLIP)
...
print(f"🎯 최종 V31-B (Model Regularization) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
