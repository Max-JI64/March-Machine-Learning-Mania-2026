...
def create_tta_features(X_va, features):
    """Test-Time Augmentation을 위해 피처 방향을 반대로 뒤집은 데이터프레임 생성"""
    X_va_swap = X_va.copy()
...
# ==============================================================================
# 2. 모델 학습 엔진 (V43 Base + TTA Inference)
# ==============================================================================
...
def train_and_eval_season(df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip_limits, weights):
    ...
    # TTA(Test-Time Augmentation)를 위한 Swap 데이터 생성
    X_va_swap = create_tta_features(X_va, features)
    
    # 1. CatBoost
    m_cat = CatBoostRegressor(**cat_p).fit(X_tr, y_tr_smooth)
    p_cat_orig = m_cat.predict(X_va)
    p_cat_swap = m_cat.predict(X_va_swap)
    p_cat = (p_cat_orig + (1 - p_cat_swap)) / 2.0  # TTA Average
    p_cat = np.clip(p_cat, 0, 1)
    
    # 2. XGBoost
    m_xgb = XGBRegressor(**xgb_p).fit(X_tr, y_tr_smooth)
    p_xgb_orig = m_xgb.predict(X_va)
    p_xgb_swap = m_xgb.predict(X_va_swap)
    p_xgb = (p_xgb_orig + (1 - p_xgb_swap)) / 2.0  # TTA Average
    p_xgb = np.clip(p_xgb, 0, 1)
    
    # 3. LightGBM
    m_lgb = LGBMRegressor(**lgb_p).fit(X_tr, y_tr_smooth)
    p_lgb_orig = m_lgb.predict(X_va)
    p_lgb_swap = m_lgb.predict(X_va_swap)
    p_lgb = (p_lgb_orig + (1 - p_lgb_swap)) / 2.0  # TTA Average
    p_lgb = np.clip(p_lgb, 0, 1)
    
    # 4. Elo
    p_elo_orig = df_train.loc[val_mask, 'EloWinProb'].values
    p_elo_swap = 1 - p_elo_orig
    p_elo = (p_elo_orig + (1 - p_elo_swap)) / 2.0
    ...
    p_calib = np.clip(iso.predict(p_blend), gender_clip_limits[0], gender_clip_limits[1])
...
if __name__ == "__main__":
    print("\n🔥 V50: V43 Baseline + Test-Time Augmentation (TTA)", flush=True)
    ...
    final_results = []
    print("\n🚀 Starting V50 Evaluation (TTA Inference)...", flush=True)
    print("| Year | Men's Brier | Women's Brier | Total Weighted Brier |", flush=True)
    
    for val_year in seasons:
        print(f"  [Year {val_year}] Training & TTA Inference...", end=' ', flush=True)
    ...
    print(f"🎯 V50 (V43 Base + Test-Time Augmentation) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
