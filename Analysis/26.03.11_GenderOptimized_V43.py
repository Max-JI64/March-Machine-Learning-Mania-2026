...
# ==============================================================================
# 2. 모델 학습 엔진 (V40 Full Data + V41 Golden Weights + 🌟 V43 Gender-Specific Features 🌟)
# ==============================================================================
# V41 Golden Weights
...
def train_and_eval_season(df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip_limits, weights):
    ...
    # feature list에서 EloWinProb가 빠졌을 경우를 대비해 df_train에서 직접 가져옴
...
# 3. 메인 프로세스
# ==============================================================================
print("\n🔥 V43: Gender-Specific Feature Selection (Noise Reduction)", flush=True)
...
# Drop Men's Noisy Features
m_drops = ['Is_Major_Conf_Diff', 'Elo_Diff', 'LateSeason_Altitude_Fatigue_Diff', '3PAr_Diff', 'OR%_Diff']
features_m = [f for f in features_m if f not in m_drops]

# Women's Base Features
features_w = [f for f in full_req_features if f in df_w.columns]
# Drop Women's Noisy Features (Zeros & Low Importance)
w_drops = ['MasMean_Diff', 'WinRate_vs_Top50_Diff', 'Upset_Value_Diff', 'Coach_Tenure_Diff', 'Rank_Trend_Slope_Diff', 'Is_Major_Conf_Diff']
features_w = [f for f in features_w if f not in w_drops]

print(f"✅ Men's 학습 피처 개수: {len(features_m)}")
print(f"✅ Women's 학습 피처 개수: {len(features_w)}")
...
print(f"\n🚀 성별 특화 최적 피처 세트 가동 중 (2012-2025)...", flush=True)
...
print(f"🎯 연속 성능 증진 V43 (Gender-Specific Optimized) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
