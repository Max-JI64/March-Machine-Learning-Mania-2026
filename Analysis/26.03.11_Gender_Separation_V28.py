...
def augment_v28(df, features, target='Label', noise_scale=0.0545):
    df_swap = df.copy()
...
# ==============================================================================
# 3. 모델 학습 엔진 (V24 Base, 성별 독립)
# ==============================================================================
def train_and_eval_season(df_train, val_year, features, cat_p, xgb_p, lgb_p):
    train_mask = (df_train['Season'] < val_year)
    val_mask = (df_train['Season'] == val_year)
    
    df_tr_aug = augment_v28(df_train[train_mask], features, 'Label', noise_scale=BEST_NOISE)
    ...
    # Extreme Clipping Trick (from V27, slightly relaxed)
    p_clipped = np.clip(p_calib, 0.02, 0.98) 
    
    return p_clipped, y_va

# ==============================================================================
# 4. 메인 프로세스 (V28: Gender Separation / M vs W Independent Training)
# ==============================================================================
print("\n🔥 V28: TRUE Gender Separation Architecture (Men's & Women's Completely Independent)", flush=True)

df_m = load_v28_data('M')
df_w = load_v28_data('W')
...
print(f"\n🚀 분리 학습 시작! (M: {len(features_m)} features, W: {len(features_w)} features)", flush=True)
print("| Year | Men's Brier | Women's Brier | Total Weighted Brier |", flush=True)

for val_year in seasons:
    # 1. Men's Training Pipeline
    p_m, y_m = train_and_eval_season(df_m, val_year, features_m, cat_params, xgb_params, lgb_params)
    s_m = brier_score_loss(y_m, p_m)
    
    # 2. Women's Training Pipeline
    p_w, y_w = train_and_eval_season(df_w, val_year, features_w, cat_params, xgb_params, lgb_params)
    s_w = brier_score_loss(y_w, p_w)
    
    # 3. COMBINE PREDICTIONS FOR FINAL EVALUATION
    # The true tournament Brier score is the average loss across all games (M & W)
...
print(f"🎯 최종 V28 (Gender Separation 구조적 분리 + Fast V16) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
