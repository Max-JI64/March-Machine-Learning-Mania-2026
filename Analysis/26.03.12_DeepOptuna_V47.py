...
# ==============================================================================
# 2. V47 Deep Optuna Objective Functions
# ==============================================================================

def get_full_los_score(df, features, model_class, params, val_years):
    scores = []
    for vy in val_years:
        ...
        if model_class == CatBoostRegressor:
            model = model_class(**params, verbose=False, random_seed=SEED).fit(X_tr, y_tr_smooth)
        elif model_class == XGBRegressor:
            model = model_class(**params, random_state=SEED, verbosity=0).fit(X_tr, y_tr_smooth)
        else: # LGBM
            model = model_class(**params, random_state=SEED, verbose=-1, n_jobs=-1).fit(X_tr, y_tr_smooth)
            
        preds = np.clip(model.predict(X_va), 1e-6, 1-1e-6)
        scores.append(brier_score_loss(y_va, preds))
    return np.mean(scores)

def objective_cat(trial, df, features, val_years):
    params = {
    ...
    return get_full_los_score(df, features, CatBoostRegressor, params, val_years)

def objective_xgb(trial, df, features, val_years):
    params = {
    ...
    return get_full_los_score(df, features, XGBRegressor, params, val_years)

def objective_lgb(trial, df, features, val_years):
    params = {
        'n_estimators': trial.suggest_int('n_estimators', 500, 2500),
        'learning_rate': trial.suggest_float('learning_rate', 1e-4, 0.2, log=True),
    ...
    return get_full_los_score(df, features, LGBMRegressor, params, val_years)

# ==============================================================================
# 3. Main Logic (V47 Deep Search)
# ==============================================================================
if __name__ == "__main__":
    print("🔥 V47: DEEP Optuna Hyperparameter Tuning for V43 Baseline (Wider Space, 100 Trials)")
    ...
    for gender, df, feats in [('Men', df_m, features_m), ('Women', df_w, features_w)]:
        print(f"\n🚀 [DEEP SEARCH] Tuning {gender} models (100 Trials per model)...")
        
        # CatBoost
        print(f"--- {gender} CatBoost ---")
        study = optuna.create_study(direction='minimize', sampler=SAMPLER)
        study.optimize(lambda t: objective_cat(t, df, feats, seasons), n_trials=N_TRIALS)
        study_results[f'{gender}_CatBoost'] = study.best_params
        print(f"Best Brier: {study.best_value:.5f}")
        
        # XGBoost
        print(f"--- {gender} XGBoost ---")
        study = optuna.create_study(direction='minimize', sampler=SAMPLER)
        study.optimize(lambda t: objective_xgb(t, df, feats, seasons), n_trials=N_TRIALS)
        study_results[f'{gender}_XGBoost'] = study.best_params
        print(f"Best Brier: {study.best_value:.5f}")
        
        # LightGBM
        print(f"--- {gender} LightGBM ---")
        study = optuna.create_study(direction='minimize', sampler=SAMPLER)
        study.optimize(lambda t: objective_lgb(t, df, feats, seasons), n_trials=N_TRIALS)
        study_results[f'{gender}_LightGBM'] = study.best_params
        print(f"Best Brier: {study.best_value:.5f}")
        
    with open('v47_deep_optuna_results.json', 'w') as f:
        json.dump(study_results, f, indent=4)
        
    print("\n✅ V47 Deep Tuning Complete. Global best parameters saved to v47_deep_optuna_results.json")
