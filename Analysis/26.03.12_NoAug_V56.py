...
def train_and_eval_season(df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip, weights):
    ...
    X_tr = df_tr[features]
    y_tr = df_tr['Label'].values.astype(float)

    all_preds = []
    for seed in SEEDS:
        set_seed(seed)

        m_cat = CatBoostRegressor(**cat_p, verbose=False, random_seed=seed).fit(X_tr, y_tr)
        p_cat = np.clip(m_cat.predict(X_va), 0, 1)

        m_xgb = XGBRegressor(**xgb_p, random_state=seed, verbosity=0).fit(X_tr, y_tr)
        p_xgb = np.clip(m_xgb.predict(X_va), 0, 1)

        m_lgb = LGBMRegressor(**lgb_p, random_state=seed, verbose=-1, n_jobs=-1).fit(X_tr, y_tr)
...
if __name__ == "__main__":
    print(f"\n🔥 V56: NO Augmentation, NO Label Smoothing + 6-Model + 10 Seeds", flush=True)
    ...
    M_CLIP, W_CLIP = (0.03, 0.97), (0.01, 0.99)
    final_results = []

    print(f"Men's features: {len(features_m)} | Women's features: {len(features_w)}", flush=True)
    print(f"Seeds: {len(SEEDS)} | Augmentation: NONE | Label Smoothing: NONE", flush=True)
    print(f"Clipping - Men: {M_CLIP} | Women: {W_CLIP}", flush=True)

    print("\n🚀 Starting V56 Evaluation...", flush=True)
    print("| Year | Men's Brier | Women's Brier | Total Weighted Brier |", flush=True)

    for val_year in seasons:
        print(f"  [Year {val_year}] Pure 6-Model x 10 Seeds...", end=' ', flush=True)
    ...
    print(f"🎯 V56 (No Aug + 6-Model + 10 Seeds) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
