...
def get_importance(df, gender):
    features = [f for f in req_features if f in df.columns]
    X, y = df[features], df['Label']
    
    # Simple training on all data to get importance
    cat = CatBoostRegressor(iterations=1000, learning_rate=0.03, depth=6, verbose=False).fit(X, y)
    xgb = XGBRegressor(n_estimators=1000, learning_rate=0.01, max_depth=5, verbosity=0).fit(X, y)
    
    cat_imp = pd.Series(cat.get_feature_importance(), index=features).sort_values(ascending=False)
    xgb_imp = pd.Series(xgb.feature_importances_, index=features).sort_values(ascending=False)
    
    # Combined normalized importance
    combined = (cat_imp / cat_imp.sum() + xgb_imp / xgb_imp.sum()) / 2
    return combined.sort_values(ascending=False)

print("--- Men's Feature Importance ---")
df_m = load_data('M')
imp_m = get_importance(df_m, 'M')
print(imp_m.to_string())

print("\n--- Women's Feature Importance ---")
df_w = load_data('W')
imp_w = get_importance(df_w, 'W')
print(imp_w.to_string())
