...
# ==============================================================================
# 1. GPU 디바이스 및 코어 설정 (LightGBM RunPod 등 GPU 환경)
# ==============================================================================
N_JOBS = os.cpu_count()
print(f"✅ 학습 알고리즘: LightGBM V2")
print(f"✅ 설정된 디바이스: CPU / CPU 코어 수: {N_JOBS}개")


# ==============================================================================
# 2. 전처리된 데이터 로드 및 병합 (A~I 파트 포함)
# ==============================================================================
def load_and_merge_features(gender='M', data_dir='../Data/preprocessed'):
    ...
    print(f"   -> [DEBUG] base_file 로드 시도: {base_file}")
    df_main = pd.read_csv(base_file)
    print(f"   -> [DEBUG] base_file 로드 완료. 현재 형태: {df_main.shape}")
    ...
    print(f"   -> [DEBUG] 검색된 추가 피처 파일 개수: {len(feature_files)}개")
    ...
    for i, file in enumerate(feature_files):
        ...
        print(f"   -> [DEBUG] 병합 진행 중 ({i+1}/{len(feature_files)}): {os.path.basename(file)}")
                    ...
                    if diff_col not in df_main.columns:
                        if f'T1_{col}' in df_main.columns and f'T2_{col}' in df_main.columns:
                            try:
                                ...
                                pass
    print(f"[{gender}] 데이터 로딩 및 병합 완료. 최종 형태: {df_main.shape}")
...
# ==============================================================================
# 3. J파트: 데이터 증강 (Data Augmentation)
# ==============================================================================
...
print(f"✅ 학습 범위: {min_season} ~ 2025")
print(f"✅ 검증 연도: {val_seasons}")


# ==============================================================================
# 5. 하이퍼파라미터 최적화 (Optuna + 튜닝 시 J파트 증강 적용)
# ==============================================================================
def objective(trial):
    # LightGBM 전용 파라미터 (1차 시기이므로 넓은 구간 탐색)
    params = {
        'n_estimators': trial.suggest_int('n_estimators', 300, 1500),
        'learning_rate': trial.suggest_float('learning_rate', 0.005, 0.05, log=True),
        'max_depth': trial.suggest_int('max_depth', 3, 8),
        'num_leaves': trial.suggest_int('num_leaves', 15, 63), # max_depth에 맞춰 2^max_depth 미만 권장
        'subsample': trial.suggest_float('subsample', 0.6, 0.9),
        'colsample_bytree': trial.suggest_float('colsample_bytree', 0.4, 0.8),
        'min_child_samples': trial.suggest_int('min_child_samples', 20, 150),
        
        # CPU 설정 (RunPod CPU Only용)
        'device_type': 'cpu',       # 기본값이지만 명시적으로 설정
    ...
    for val_year in val_seasons[-3:]:
        ...
        model.fit(
            X_tr_aug, y_tr_smooth, 
            sample_weight=weights_aug,
...
print("\n🚀 1단계: 하이퍼파라미터 튜닝 (LightGBM 120 Cores, 증강 데이터 활용, 30 trials)...")
...
for val_year in val_seasons:
    ...
    model.fit(X_tr_aug, y_tr_smooth, sample_weight=weights_aug)
