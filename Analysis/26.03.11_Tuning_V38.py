...
# ==============================================================================
# 1. 데이터 베이스 및 피처 구성 (V37 플레이스타일 주입 기반 유지)
# ==============================================================================
...
def augment_v38(df, features, target='Label', noise_scale=0.0545):
    df_swap = df.copy()
...
def train_and_eval_season(df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip_limits):
    ...
    df_tr_aug = augment_v38(df_train[train_mask], features, 'Label', noise_scale=BEST_NOISE)
...
# 3. 메인 프로세스
# ==============================================================================
print("\n🔥 V38: Hyperparameter Fine-Tuning for 0.12 Target", flush=True)

df_m = load_v38_data('M')
df_w = load_v38_data('W')

with open('selected_features_v7.json', 'r') as f:
    v7_features = json.load(f)

# V37 필수 피처 세트 + 3차 분석 팩터
...
# 🌟 V38 TUNED HYPERPARAMETERS 🌟
# 피처가 41개로 늘어남에 따라 발생할 수 있는 노이즈를 제어하기 위한 타이트한 정규화 튜닝
cat_params = {
    'iterations': 1200,                # 1037 -> 1200 (조금 더 길게 학습)
    'learning_rate': 0.02,             # 0.0292 -> 0.02 (천천히 학습)
    'depth': 5,                        # 6 -> 5 (깊이를 줄여 과적합 방지)
    'l2_leaf_reg': 15.0,               # 10.09 -> 15.0 (정규화 강화)
    'subsample': 0.85,                 # 0.898 -> 0.85
    'random_strength': 1.0,            # 0.754 -> 1.0 (스플릿의 무작위성 증가시켜 노이즈 방어)
    'bagging_temperature': 0.8,        # 0.614 -> 0.8 (배깅 다양성 증가)
    'border_count': 128, 'thread_count': -1, 'random_seed': SEED, 'verbose': False
}

xgb_params = {
    'n_estimators': 1200,              # 1000 -> 1200
    'learning_rate': 0.015,            # 0.01 -> 0.015
    'max_depth': 4,                    # 5 -> 4 (CatBoost처럼 트리 깊이를 낮춤)
    'colsample_bytree': 0.7,           # 1.0 -> 0.7 (41개 피처 중 70%만 랜덤 샘플링하여 분산 감소)
...
lgb_params = {
    'n_estimators': 1200, 
    'learning_rate': 0.015, 
    'num_leaves': 20,                  # 31 -> 20 (리프 노드 개수를 대폭 줄여 단순한 트리 생성)
    'colsample_bytree': 0.7,           # 피처 샘플링 비중 추가
...
print(f"🎯 연속 성능 증진 V38 (0.12 Target Tuning) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
