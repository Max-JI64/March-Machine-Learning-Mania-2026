...
def create_tta_features(X_va, features):
    X_va_swap = X_va.copy()
    diff_cols = [c for c in X_va.columns if c.endswith('_Diff') and c in features]
    for dc in diff_cols: X_va_swap[dc] = -X_va_swap[dc]
    for pc in ['EloWinProb', 'SeedWinProb']:
        if pc in features: X_va_swap[pc] = 1 - X_va_swap[pc]
    for f in ['Momentum_Upset_Risk', 'Tournament_DNA_Risk']:
        if f in features: X_va_swap[f] = -X_va_swap[f]
    return X_va_swap
...
# ==============================================================================
# 2. 모델 학습 엔진 (Leakage-Free Calibration + TTA)
# ==============================================================================
...
def train_and_eval_season(df_train, val_year, features, cat_p, xgb_p, lgb_p, gender_clip_limits, weights):
    ...
    # 2. Get Training Predictions for Calibration (Leakage Free!)
    p_cat_tr = np.clip(m_cat.predict(X_tr), 0, 1)
    p_xgb_tr = np.clip(m_xgb.predict(X_tr), 0, 1)
    p_lgb_tr = np.clip(m_lgb.predict(X_tr), 0, 1)
    p_elo_tr = df_tr_aug['EloWinProb'].values
    p_blend_tr = np.clip((weights[0]*p_cat_tr + weights[1]*p_xgb_tr + weights[2]*p_lgb_tr + weights[3]*p_elo_tr), 0, 1)
    
    # Platt Scaling (Logistic Regression) on Training Data
    # y_tr is the original 0/1 labels from df_tr_aug
    calibrator = LogisticRegression(C=1.0, penalty='l2', solver='lbfgs')
    calibrator.fit(p_blend_tr.reshape(-1, 1), y_tr)
    
    # 3. Predict on Validation with TTA
    X_va_swap = create_tta_features(X_va, features)
    
    p_cat_va = (m_cat.predict(X_va) + (1 - m_cat.predict(X_va_swap))) / 2.0
    p_xgb_va = (m_xgb.predict(X_va) + (1 - m_xgb.predict(X_va_swap))) / 2.0
    p_lgb_va = (m_lgb.predict(X_va) + (1 - m_lgb.predict(X_va_swap))) / 2.0
    
    p_elo_orig = df_train.loc[val_mask, 'EloWinProb'].values
    p_elo_va = (p_elo_orig + (1 - (1 - p_elo_orig))) / 2.0
    
    p_blend_va = np.clip((weights[0]*p_cat_va + weights[1]*p_xgb_va + weights[2]*p_lgb_va + weights[3]*p_elo_va), 0, 1)
    
    # 4. Apply Calibration
    p_calib = calibrator.predict_proba(p_blend_va.reshape(-1, 1))[:, 1]
    p_calib = np.clip(p_calib, gender_clip_limits[0], gender_clip_limits[1])
...
if __name__ == "__main__":
    print("\n🔥 V51: True Baseline (Leakage-Free Platt Scaling) + TTA", flush=True)
    ...
    cat_params = {'iterations': 1037, 'learning_rate': 0.0292, 'depth': 6, 'l2_leaf_reg': 10.09, 'subsample': 0.898, 'random_strength': 0.754, 'bagging_temperature': 0.614, 'border_count': 128, 'thread_count': -1, 'random_seed': SEED, 'verbose': False}
    ...
    final_results = []
    print("\n🚀 Starting V51 Evaluation...", flush=True)
    print("| Year | Men's Brier | Women's Brier | Total Weighted Brier |", flush=True)
    
    for val_year in seasons:
        print(f"  [Year {val_year}] Training & Calibrating...", end=' ', flush=True)
        
        p_m, y_m = train_and_eval_season(df_m, val_year, features_m, cat_params, xgb_params, lgb_params, M_CLIP, M_WEIGHTS)
        s_m = brier_score_loss(y_m, p_m)
        
        try:
            if val_year in df_w['Season'].values:
                p_w, y_w = train_and_eval_season(df_w, val_year, features_w, cat_params, xgb_params, lgb_params, W_CLIP, W_WEIGHTS)
    ...
    print(f"🎯 V51 (True Baseline: Leakage-Free Platt Scaling + TTA) 평균 Brier Score: {np.mean(final_results):.5f}", flush=True)
