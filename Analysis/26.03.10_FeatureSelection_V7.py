...
# ==============================================================================
# 2. CatBoost Feature Importance 추출
# ==============================================================================
print(f"🚀 1단계: 전체 데이터 기반 Feature Importance 추출 (Total Features: {len(features)})")

# CatBoost V6 최고 성능 모델 파라미터 재사용
cat_params = {
...
# 가장 중요도가 높을 최근 5년(2019-2025)을 기준으로 Importance 산출
val_seasons_imp = [2019, 2021, 2022, 2023, 2024, 2025]
feature_importances = np.zeros(len(features))

for val_year in val_seasons_imp:
    ...
    train_aug = augment_data(train_temp, features, target, noise_scale=0.0209)
    weights_aug = get_recency_sample_weights(train_aug['Season'], max_season=val_year)
    
    # Label Smoothing
    y_tr_smooth = train_aug[target] * (1 - 0.05) + 0.5 * 0.05
    
    model_cat = CatBoostRegressor(**cat_params)
    model_cat.fit(train_aug[features], y_tr_smooth, sample_weight=weights_aug)
    
    feature_importances += model_cat.get_feature_importance() / len(val_seasons_imp)
...
print("\n--- BOTTOM 10 Features (Noise Candidates) ---")
print(feat_imp_df.tail(10))

# 하위 50%의 잉여 피처들 과감하게 드롭 (Top N 피처만 선택)
keep_ratio = 0.50
keep_n = int(len(features) * keep_ratio)
top_features = feat_imp_df['Feature'].head(keep_n).tolist()

dropped_features = feat_imp_df['Feature'].tail(len(features) - keep_n).tolist()

print(f"\n🚀 2단계: 피처 컷오프 실행")
print(f"✅ 유지되는 피처 (Top {keep_n}개): {len(top_features)}")
print(f"❌ 드롭되는 노이즈 피처 수: {len(dropped_features)}")

# 리스트를 저장해서 Stacking 스크립트가 로드할 수 있게 함
import json
with open('selected_features_v7.json', 'w') as f:
    json.dump(top_features, f)

print(f"✅ 선택된 피처 리스트가 'selected_features_v7.json'에 성공적으로 저장되었습니다.")
